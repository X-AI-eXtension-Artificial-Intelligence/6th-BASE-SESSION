...
# 메모리 문제로 실행이 안돼서 데이터를 줄여서 실행 
    def make_dataset(self):
        data_root = './multi30k_data' 

        train_data = list(Multi30k(root=data_root, split='train', language_pair=self.ext))[:100]
        valid_data = list(Multi30k(root=data_root, split='valid', language_pair=self.ext))[:20]
        test_data = list(Multi30k(root=data_root, split='test', language_pair=self.ext))[:20]
        return train_data, valid_data, test_data
