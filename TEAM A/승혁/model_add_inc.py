...
def conv_1x1_block(in_dim, out_dim):
    model = nn.Sequential(
        nn.Conv2d(in_dim, out_dim, kernel_size=1),  # 1x1 컨볼루션
        nn.ReLU(),                                  # 활성화 함수 (ReLU)
    )
    return model
...
class VGG(nn.Module):
    ...
    def __init__(self, base_dim, num_classes=10):
        ...
        self.feature = nn.Sequential(
            ...
            conv_1x1_block(8*base_dim, 4*base_dim)
        )
        
        self.fc_layer = nn.Sequential(
            # CIFAR-10은 입력 이미지 크기가 32x32이므로 1x1 feature map 사용
            nn.Linear(4*base_dim*1*1, 4096),  # 512 -> 4096 Fully Connected Layer
