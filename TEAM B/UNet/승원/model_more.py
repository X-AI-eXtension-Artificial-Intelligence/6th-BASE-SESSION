...
class UNet(nn.Module):  # nn.Module을 상속하여 UNet 클래스 정의
    def __init__(self):
        ...
        self.enc5_1 = CBR2d(512, 1024)  # 512 → 1024
        self.enc5_2 = CBR2d(1024, 1024)  # 1024 → 1024
        self.pool5 = nn.MaxPool2d(kernel_size=2)  # 풀링

        self.enc6_1 = CBR2d(1024, 2048)  # 가장 깊은 층 (보틀넥) 1024 →2048

        # Expansive path


        self.dec6_1 = CBR2d(2048, 1024)  # 2048 → 1024
        self.unpool5 = nn.ConvTranspose2d(1024, 1024, kernel_size=2, stride=2, padding=0)  # 업샘플링 (2배 확대)

        self.dec5_2 = CBR2d(2048, 1024)  # 1024 + 1024(Skip Connection) → 1024
        self.dec5_1 = CBR2d(1024, 512)  # 1024 → 512
        self.unpool4 = nn.ConvTranspose2d(512, 512, kernel_size=2, stride=2, padding=0)  # 업샘플링
    ...
    def forward(self, x):
        ...
        dec5 = torch.cat((unpool5, enc5_2), dim=1)
        dec5 = self.dec5_2(dec5)
