...
## Parser 생성하기
# 하드 코딩을 피하고 유연한 실행 환경을 제공
# ex. !python train.py --lr 5e-4
# ex. !python train.py --help
...
# transpose(0, 2, 3, 1): (N, C, H, W) → (N, H, W, C)로 변환 (이미지 데이터 표준 형태)

fn_denorm = lambda x, mean, std: (x * std) + mean # 정규화된 데이터를 원래 값으로 되돌리는 함수
fn_class = lambda x: 1.0 * (x > 0.5) # 모델의 예측값을 0 또는 1로 변환하는 함수 (이진 분류)
